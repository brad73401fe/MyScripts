...
def RUN_MAIN_5B(FILE_NAME_HEADER,HEADER_ATTRIB,HEADER_FGCOLR,HEADER_BGCOLR,HEADER_RSTALL):
	...
	BAND_5_INPUT = raw_input("Color of band #5: ");
	print("");
	...
	if (DebugEnable == 1):
		...
		print("%-16s = %s"       % ("BAND_5_INPUT", BAND_5_INPUT));
		print("");
	...
	#Check to make sure band 5 input is valid.
	try:
		print("%-16s = %s"       % ("BAND_5_NUMB", BAND_5_DICT[BAND_5_INPUT]));
	except:
		ExitStatus0=1;
		print("%-16s = %s%s%s"   % ("BAND_5_NUMB", "\x1b[31m", "Not a valid color. Try again.", "\x1b[0m"));
	print("");
	...
	BAND_5_NUMB = BAND_5_DICT[BAND_5_INPUT];
	
	FIVE_BAND_NUMB = (((BAND_1_NUMB * 100) + (BAND_2_NUMB * 10) + BAND_3_NUMB) * BAND_4_NUMB);
	print("%-16s = %s %s  (%s)" % ("FIVE_BAND_NUMB", '{:,.2f}'.format(FIVE_BAND_NUMB), u"\u03A9", BAND_5_NUMB));
#END RUN_MAIN_5B();
